...
- `krate(name) { }` — the top-level entry point
...
- `Store<K, T>` interface with `add`, `delete`, `update`, `asFlow`, `observe`, `findByPredicate`, `aggregate`, etc.
...
- `CrateMigration` DSL for schema migrations
...
Provides `inMemoryKrate { }` — an in-memory `Krate` backed by `MutableMap`s. Perfect for unit testing repositories and ViewModels without a real database.
